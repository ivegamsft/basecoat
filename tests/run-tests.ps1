...
Write-Host 'Running licensing posture tests...'
& pwsh -NoProfile -File (Join-Path $PSScriptRoot 'licensing-posture-tests.ps1')
if ($LASTEXITCODE -ne 0) {
    Write-Host 'Licensing posture tests failed' -ForegroundColor Red
    Write-FailureLog 'licensing-posture-tests'
    exit 1
}
