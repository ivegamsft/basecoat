...
db/backup-scripts/backup.sh daily

# Weekly backup (12-week retention)
db/backup-scripts/backup.sh weekly

# Monthly backup (24-month retention)
db/backup-scripts/backup.sh monthly
```
...
```bash
# Daily at 2 AM
0 2 * * * /path/to/basecoat/db/backup-scripts/backup.sh daily

# Weekly Sunday at 3 AM
0 3 * * 0 /path/to/basecoat/db/backup-scripts/backup.sh weekly

# Monthly 1st at 4 AM
0 4 1 * * /path/to/basecoat/db/backup-scripts/backup.sh monthly
```
...
db/backup-scripts/restore.sh /backups/daily/basecoat_portal_20250505.dump

# Restore to alternate database
db/backup-scripts/restore.sh /backups/daily/basecoat_portal_20250505.dump staging_db
```
...
```bash
# Create directory
mkdir -p db/migrations/v1.2

# Create migration file with timestamp ordering
cat > db/migrations/v1.2/001_add_feature_x.sql << 'EOF'
