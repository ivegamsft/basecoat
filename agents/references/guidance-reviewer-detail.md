# Guidance Reviewer — Detail

Supporting detail for [`agents/basecoat-90-quality-guidance-reviewer.agent.md`](../basecoat-90-quality-guidance-reviewer.agent.md).

## Frontmatter Requirements by Asset Type

- Instructions: `description`, `applyTo`
- Skills: `name`, `description`, `compatibility`
- Agents: `name`, `description`, `visibility`. `allowed_skills` is optional; `model:` is a legacy
  optional field, and `model_policy`/capability metadata are recommended rather than required.
...
- Agent `name` must match the short-name portion of the file base name: strip the
  `basecoat-NN-<category>-` prefix; the remaining filename base (before `.agent.md`) must equal
  `name:`. Example: `basecoat-10-core-agent-designer.agent.md` → `name: agent-designer`. Bare
  filenames like `guidance-reviewer.agent.md` must match `name: guidance-reviewer` exactly.

## Scope and Quality Assessment

- Is the description using `USE FOR:` / `DO NOT USE FOR:` format per BaseCoat convention (see
