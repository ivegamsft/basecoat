...
- **Consumer update policy** --- how release drift is reported or delivered.
...
| **Required** | No |
| **Default** | `main` |
| **Valid values** | Any git branch name or release tag (e.g. `v4.2.1`) |
...
Optional corporate clone endpoint. Release notes and policy identity remain tied
to `source`; immutable tag/SHA resolution and sync fetches use `mirror`.
...
Optional release remapping consumed by both sync and the updater.
...
The `updates` map controls the scheduled consumer updater. Defaults are
`mode: notify`, `approval: required`, stable channel, and patch/minor bumps.
...
|---|---|---|
| `channel` | `stable` | Release channel; unsupported channels fail closed |
| `cadence` | `weekly` | Documented schedule intent used by the template |
| `mode` | `notify` | Stable issue or upgrade pull request |
| `approval` | `required` | Required review or GitHub auto-merge |
| `allowed_bumps` | `[patch, minor]` | SemVer bump classes eligible for PR delivery |
| `source` | top-level `source` | Canonical release repository |
| `ref` | `latest` | Latest stable release or explicit tag |
| `mirror` | top-level `mirror` | Corporate fetch endpoint |
| `validation` | unset | Optional consumer validation command |

Automatic approval never uses admin bypass. GitHub required checks, branch
protection, required reviews, and merge queues still govern the merge. Major
upgrades always use required approval, even if `automatic` was configured.

`pull-request` mode also requires the reusable workflow's named `update_token`
secret, normally forwarded from repository secret `BASECOAT_UPDATE_TOKEN`. The
recommended setup is a dedicated fine-grained PAT scoped to the consumer
repository. The updater rejects the default `GITHUB_TOKEN` for PR delivery
because events generated by that token do not trigger normal consumer push and
pull-request workflows.

Grant the PAT **Actions: read**, **Checks: read**, **Commit statuses: read**,
**Contents: read and write**, **Issues: read and write**, and **Pull requests:
read and write** repository permissions. Pull-request mode uses this credential
as `GH_TOKEN` for issue maintenance, check observation, and all branch and
pull-request delivery operations. Release resolution uses anonymous public
lookup or the separately authority-bound fetch credential.

GitHub App installation tokens are short-lived and must not be persisted as
repository secrets. If GitHub App authentication is preferred, generate an
installation token at workflow runtime and pass that runtime output to the
callable workflow's `update_token` input. Set `update_actor` to the app bot
login (for example, `basecoat-updater[bot]`) to bind generated PR ownership.

Private canonical source authentication uses read-only `fetch_token` with
`fetch_host` exactly matching the source HTTPS authority. A private mirror on a
different authority uses separate `mirror_fetch_token` and
`mirror_fetch_host`; the canonical token is reusable only when source and
mirror authorities match. Consumer delivery credentials are never forwarded to
a source or mirror.
...
# .basecoat.yml
source: https://github.com/YOUR-ORG/basecoat.git
ref: v4.2.1   # pinned --- update deliberately after reviewing the changelog
