...
This document maps the 24 in-scope workflows for Workstream 4 (first checkpoint) to risk tiers according to the [Risk-Tier Autonomy Policy](../reference/risk-tier-policy.md).
...
|---|---|---|---|---|---|
| issue-triage.lock.yml | 1 | Read-only analysis; no side effects | None | None | N/A |
| code-review-agent.lock.yml | 1 | Read-only feedback; draft PR comments only | None | None | N/A |
| security-analyst.lock.yml | 1 | Read-only security analysis; advisory comments | None | None | N/A |
| release-impact-advisor.lock.yml | 1 | Read-only impact analysis; report only | None | None | N/A |
...
| enforce-protection.yml | 4 | Protection enforcement can immediately gate/unblock release paths | Two-Person Approval (Admin + On-Call) | Sync | Manual by on-call + audit |
...
| self-healing-ci.lock.yml | 2 | Auto-fixes CI failures on feature branches | PR Review | 8h | Git revert |
...
| retro-facilitator.lock.yml | 1 | Weekly retrospective analysis; no production side effects | None | None | N/A |
...
| Tier | Count | Examples |
|------|-------|----------|
| **1** (Read-only, auto-execute) | 11 | issue-triage.lock, code-review-agent.lock, dependency-audit |
| **2** (Feature branch, PR review) | 8 | ci, pr-validation, docs staging |
| **3** (Production, explicit approval) | 4 | docs-production, publish-to-production, terraform-deploy |
| **4** (Critical, two-person rule) | 1 | enforce-protection |

**Total in-scope workflows classified**: 24  
**Coverage %**: 100%
...
- [ ] Risk-tier labels enforced on PRs and issues that modify workflows (`risk-tier:1`, etc.)
...
1. **Phase 2** — Require `risk-tier:N` labels on PRs/issues that modify workflow files (`.github/workflows/*.yml`)
...
**Q: Should retro-facilitator.lock.yml (AI-generated retrospectives) be Tier 2 or higher?**  
