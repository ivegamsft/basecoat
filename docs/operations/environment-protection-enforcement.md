...
| Production manual approvals configured | GitHub environment API check (`prod`) | Per PR + weekly |
| Production branch restrictions configured | GitHub environment API check (`prod`) | Per PR + weekly |
...
2. Confirm `prod` environment has required reviewers configured.
3. Confirm `prod` environment deployment branch policy is enabled.
4. Confirm deployment workflows use `environment:` for deployment jobs.
5. Confirm production-targeting jobs resolve to `prod` and consume secrets/vars from protected scope.
...
- Environment protection screenshot or API output for `prod`
